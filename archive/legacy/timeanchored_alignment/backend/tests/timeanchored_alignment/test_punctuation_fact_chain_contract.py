...
def _build_ready_window() -> ReadySlowWindow:
    return ReadySlowWindow(
        window_id="window-punct-chain-001",
...
def test_preparation_keeps_punctuation_evidence_when_slow_text_is_unpunctuated_but_punct_track_exists() -> None:
...
def test_anchor_mount_receives_sentence_end_punctuation_facts_from_preparation() -> None:
    punct_track = PunctTrack(
        clean_text_ref="你好世界",
        positions=[PuncPosition(char_index=3, punctuation="。", confidence=0.99)],
        source="fast",
    ...
    ingress = AnchorMountAlignmentService().align(preparation=package, language="zh").decision_ingress
    assert any(fact.punct_class == "sentence_end" for fact in ingress.punctuation_facts)
