"""Seed-and-chain 的初始锚点发现。"""

from __future__ import annotations

from app.services.timeanchored_alignment.anchor_mount.contracts import (
    AnchorCandidate,
    AnchorMountInputView,
)


def _normalize(text: str) -> str:
    return "".join(
        char.lower()
        for char in str(text or "")
        if char.isalnum() or ("\u4e00" <= char <= "\u9fff")
    )


class SeedDiscovery:
    """从 slots 与 hooks 中提取 hard/soft seeds。"""

    def discover(self, *, input_view: AnchorMountInputView) -> tuple[AnchorCandidate, ...]:
        hints_by_key = {
            _normalize(hint.token_text): hint
            for hint in input_view.pronunciation_hints
            if _normalize(hint.token_text)
        }
        candidates: list[AnchorCandidate] = []
        for slot_index, slot in enumerate(input_view.slots):
            slot_key = _normalize(slot.text)
            if not slot_key:
                continue
            matched = False
            for hook_index, hook in enumerate(input_view.fast_hooks):
                hook_key = _normalize(hook.hook_text)
                if slot.text == hook.hook_text:
                    candidates.append(
                        AnchorCandidate(
                            slot_indices=(slot_index,),
                            hook_indices=(hook_index,),
                            anchor_kind="exact",
                            score=1.0,
                            is_hard=True,
                        )
                    )
                    matched = True
                elif slot_key and slot_key == hook_key:
                    candidates.append(
                        AnchorCandidate(
                            slot_indices=(slot_index,),
                            hook_indices=(hook_index,),
                            anchor_kind="normalized",
                            score=0.88,
                            is_hard=True,
                        )
                    )
                    matched = True
            if not matched and slot_key in hints_by_key:
                for hook_index, hook in enumerate(input_view.fast_hooks):
                    if slot_key == _normalize(hook.hook_text):
                        candidates.append(
                            AnchorCandidate(
                                slot_indices=(slot_index,),
                                hook_indices=(hook_index,),
                                anchor_kind="soft_pronunciation",
                                score=0.65,
                                is_hard=False,
                            )
                        )
            if not matched and len(input_view.slots) > 1:
                for next_slot_index in range(slot_index + 1, len(input_view.slots)):
                    merged_key = _normalize(slot.text + input_view.slots[next_slot_index].text)
                    for hook_index, hook in enumerate(input_view.fast_hooks):
                        if merged_key and merged_key == _normalize(hook.hook_text):
                            candidates.append(
                                AnchorCandidate(
                                    slot_indices=(slot_index, next_slot_index),
                                    hook_indices=(hook_index,),
                                    anchor_kind="merge",
                                    score=0.82,
                                    is_hard=True,
                                )
                            )
            if not matched:
                sequence_candidate = self._match_contiguous_hook_sequence(
                    slot_key=slot_key,
                    hooks=input_view.fast_hooks,
                    slot_index=slot_index,
                )
                if sequence_candidate is not None:
                    candidates.append(sequence_candidate)
        return tuple(candidates)

    @staticmethod
    def _match_contiguous_hook_sequence(
        *,
        slot_key: str,
        hooks,
        slot_index: int,
    ) -> AnchorCandidate | None:
        if not slot_key:
            return None

        normalized_hooks = [
            _normalize(getattr(hook, "hook_text", ""))
            for hook in hooks
        ]
        for start_index in range(len(normalized_hooks)):
            merged_parts: list[str] = []
            hook_indices: list[int] = []
            for hook_index in range(start_index, len(normalized_hooks)):
                hook_key = normalized_hooks[hook_index]
                if not hook_key:
                    continue
                merged_parts.append(hook_key)
                hook_indices.append(hook_index)
                merged_key = "".join(merged_parts)
                if merged_key == slot_key:
                    return AnchorCandidate(
                        slot_indices=(slot_index,),
                        hook_indices=tuple(hook_indices),
                        anchor_kind="sequence",
                        score=0.9,
                        is_hard=True,
                    )
                if len(merged_key) > len(slot_key):
                    break
        return None
